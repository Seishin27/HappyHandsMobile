...
// ─────────────────────────────────────────────────────────────────────────────
// Rider Auth Screen  (Login + Register)
// ─────────────────────────────────────────────────────────────────────────────
...
class _RiderAuthScreenState extends State<RiderAuthScreen> {
  ...
  Widget build(BuildContext context) {
    return AuthShell(
      illustration: _RiderIllustration(),
  ...
  Widget _buildLoginCard() {
    return Column(
      ...
      children: [
        ...
        const Text('Deliver orders and earn flexible income.',
        ...
        Center(child: TextButton(
          ...
          child: const Text('Back to home', style: TextStyle(fontSize: 13, color: Color(0xFF64748B))),
        )),
      ],
    );
  }
    ...
    return Column(
      ...
      children: [
        const Text('Become a Rider',
            style: TextStyle(fontSize: 26, fontWeight: FontWeight.w800, color: Color(0xFF0F172A))),
        const SizedBox(height: 6),
        const Text('Start delivering today.',
            style: TextStyle(fontSize: 14, color: Color(0xFF64748B))),
        const SizedBox(height: 24),
        _RiderRegisterForm(onSuccess: (msg) => _showSuccessAndGoLogin(msg)),
        const SizedBox(height: 20),
        Center(child: RichText(text: TextSpan(
          style: const TextStyle(fontSize: 13, color: Color(0xFF64748B)),
          children: [
    ...
    return Image.asset(
      ...
      errorBuilder: (_, __, ___) => const Column(
        ...
        children: [
          ...
          SizedBox(height: 16),
          Text('Rider Portal',
              style: TextStyle(fontSize: 26, fontWeight: FontWeight.w800, color: Color(0xFF2C5AA0))),
        ],
      ),
    );
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// Rider Registration Form  (3-step wizard)
// ─────────────────────────────────────────────────────────────────────────────
class _RiderRegisterForm extends StatefulWidget {
  final void Function(String msg)? onSuccess;
  const _RiderRegisterForm({this.onSuccess});

  @override
  State<_RiderRegisterForm> createState() => _RiderRegisterFormState();
}

class _RiderRegisterFormState extends State<_RiderRegisterForm> {
  int _step = 0;
  ...
  String? _vehicleType;
  File? _licenseFile;           // mobile
  Uint8List? _licenseBytes;     // web
  ...
  static const _vehicles = ['Motorcycle', 'Car', 'Van'];
  ...
  @override
  void dispose() {
    for (final c in [_nameCtrl, _emailCtrl, _phoneCtrl, _passCtrl, _confirmCtrl]) { c.dispose(); }
    super.dispose();
  }
  ...
  void _next() { if (_validate(_step)) setState(() { _error = null; _step++; }); }
  ...
  Future<void> _submit() async {
    ...
    if (_licenseFile == null && _licenseBytes == null) {
      setState(() => _error = "Please upload your Driver's License to continue.");
      return;
    }
    setState(() { _loading = true; _error = null; });
    try {
      final uri = Uri.parse('${AppConfig.apiBaseUrl}/rider/register');

      // Always send JSON — bytes pre-loaded via withData:true
      final body = <String, dynamic>{
        'ridername':        _nameCtrl.text.trim(),
        'rideremail':       _emailCtrl.text.trim(),
        'phone':            _phoneCtrl.text.trim(),
        'riderpass':        _passCtrl.text,
        'confirmriderpass': _confirmCtrl.text,
        'region':           _region?.name ?? '',
      ...
      if (_licenseBytes != null) {
        body['driverlicense_base64'] = base64Encode(_licenseBytes!);
        body['driverlicense_filename'] = _licenseFileName ?? 'license.jpg';
      } else if (_licenseFile != null) {
        final bytes = await _licenseFile!.readAsBytes();
        body['driverlicense_base64'] = base64Encode(bytes);
        body['driverlicense_filename'] = _licenseFileName ?? 'license.jpg';
      }
      ...
      if (res.statusCode == 201 || json['status'] == 'success') {
        final msg = json['message']?.toString() ?? 'Application submitted! Please wait for approval.';
        if (mounted) widget.onSuccess?.call(msg);
      } else {
        setState(() => _error = json['message']?.toString() ?? 'Registration failed.');
      }
    } catch (e) {
  ...
  InputDecoration _dec({required String label, required String hint, required IconData icon, Widget? suffix}) =>
      authFieldDec(label: label, hint: hint, icon: icon, suffix: suffix);
  ...
  Widget _stepIndicator() {
    const labels = ['Account', 'Location', 'Vehicle'];
    return Padding(
  ...
  Widget _navButtons(String nextLabel) => Row(children: [
    if (_step > 0) ...[
  ...
  Widget _step1() => Form(key: _s1Key, child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
    ...
    TextFormField(controller: _emailCtrl, keyboardType: TextInputType.emailAddress,
        decoration: _dec(label: 'Email', hint: 'you@example.com', icon: FontAwesomeIcons.envelope),
        validator: (v) {
  ...
  Widget _step3() => Form(key: _s3Key, child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
    _sectionLabel('VEHICLE & LICENSE'),
    DropdownButtonFormField<String>(
      initialValue: _vehicleType,
      decoration: _dec(label: 'Vehicle Type', hint: 'Select vehicle type', icon: FontAwesomeIcons.motorcycle),
      items: _vehicles.map((t) => DropdownMenuItem(value: t, child: Text(t))).toList(),
    ...
    GestureDetector(
      onTap: () async {
        if (kIsWeb) {
          // Web: use FilePicker to get bytes directly
          ...
          if (picked != null && mounted) {
            // Try to read bytes directly for consistency
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: (_licenseFile != null || _licenseBytes != null) ? AppTheme.primaryBlue : const Color(0xFFE2E8F0),
            width: (_licenseFile != null || _licenseBytes != null) ? 1.5 : 1,
          ),
        ),
        child: Row(children: [
          Icon(
            (_licenseFile != null || _licenseBytes != null) ? FontAwesomeIcons.fileCircleCheck : FontAwesomeIcons.fileArrowUp,
            color: (_licenseFile != null || _licenseBytes != null) ? AppTheme.primaryBlue : const Color(0xFF94A3B8),
            size: 18,
          ),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(
              (_licenseFile != null || _licenseBytes != null) ? "License selected ✓" : "Upload Driver's License",
              style: TextStyle(
                fontWeight: FontWeight.w600,
                fontSize: 13,
                color: (_licenseFile != null || _licenseBytes != null) ? AppTheme.primaryBlue : const Color(0xFF0F172A),
              ),
            ),
            const SizedBox(height: 2),
            Text(
              (_licenseFile != null || _licenseBytes != null)
                  ? (_licenseFileName ?? 'File selected')
          ...
          if (_licenseFile != null || _licenseBytes != null)
            GestureDetector(
              onTap: () => setState(() { _licenseFile = null; _licenseBytes = null; _licenseFileName = null; }),
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const Expanded(child: Text.rich(TextSpan(
          ...
          children: [
            ...
            TextSpan(text: 'Rider Terms and Conditions',
    ...
    Container(
      ...
      child: const Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Icon(FontAwesomeIcons.circleInfo, color: Color(0xFF94A3B8), size: 13),
        SizedBox(width: 8),
        Expanded(child: Text(
